...
public class Tinker3Agent : Agent
{
    ...
    int poseTypeIndex = 0;  //索引
    int poseCounter = 0;    //当前计数
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if(train){
            float poseRandProb = Academy.Instance.EnvironmentParameters.GetWithDefault("pose_random_prob", 1);
            if(poseRandProb==1){
                print("poseRandProb==1");
                float randomValue = Random.Range(0f, 1f);
                float randomY = Random.Range(-180f, 180f);
                if (randomValue < 0.5f)
                {    
                    // 脸朝地 (正常趴着) -> Z轴 0度         
                    randRot = rot0 * Quaternion.Euler(90f, randomY, 0f);
                }
                else
                {
                    // 仅侧躺随机：X 保持水平，仅随机 Y 轴朝向与 Z 轴侧倒
                    float sideAngle = Random.Range(0, 2) == 0 ? -90f : 90f;
                    randRot = rot0 * Quaternion.Euler(0f, Random.Range(-180f, 180f), sideAngle);
                }
            }
            else{
                print("poseRandProb=2");
                float randomValue = Random.Range(0f, 1f);
                float randomY = Random.Range(-180f, 180f);
                if (randomValue <= 0.6f)
                {    
                    randRot = rot0 * Quaternion.Euler(Random.Range(-80f, 80f), Random.Range(-180f, 180f), -90);
                }
        ...
        // Quaternion randRot;
        // // 姿态随机概率由环境参数控制，便于通过 curriculum 调节
        // float poseRandProb = Academy.Instance.EnvironmentParameters.GetWithDefault("pose_random_prob", 0.3f);
        // if (Random.Range(0f, 1f) < poseRandProb)
        // {
        //     // 完全随机：X、Y、Z 都在较大范围内随机
        // randRot = rot0 * Quaternion.Euler(Random.Range(-80f, 80f), Random.Range(-180f, 180f), -90);
        // }
        ...
        if(!train){
            // Quaternion randRot = rot0;
            // 每次 Y 轴（水平朝向）都随机，保证多样性
            float randomY = Random.Range(-180f, 180f);

            switch (poseTypeIndex)
            {
                case 0: // 平躺 (仰面朝天) -> Z轴 180度
                    randRot = rot0 * Quaternion.Euler(-90f, randomY, 0f);
                    break;

                case 1: // 脸朝地 (正常趴着) -> Z轴 0度
                    randRot = rot0 * Quaternion.Euler(90f, randomY, 0f);
                    break;

                case 2: // 侧躺 (随机左侧或右侧) -> Z轴 -90 或 90度
                    float sideAngle = Random.Range(0, 2) == 0 ? -90f : 90f;
                    randRot = rot0 * Quaternion.Euler(0f, randomY, sideAngle);
                    break;
            ...
            // 更新计数器和姿势索引
            poseCounter++; 
            if (poseCounter >= 3) // 如果已经跑了3次
            {
                poseCounter = 0; // 计数归零
                poseTypeIndex++; // 切换到下一个姿势
                if (poseTypeIndex > 3) poseTypeIndex = 0; // 如果超过3(随机)，回到0(平躺)
            }
        }
    ...
    void FixedUpdate()
    {
        if (train)
        {
            // 动态从 ML-Agents 环境参数读取 fy（牵引力大小）
            fy = Academy.Instance.EnvironmentParameters.GetWithDefault("lift_force", 70f);
        }
        ...
        // arts[0].AddForce(ffy , ForceMode.Force)；
        // 计算cube的up坐标轴与竖直轴的点积
